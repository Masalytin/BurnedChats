...
describe('IMP-TNFS-04 matrix check helpers — fee 0.5/0.3/0.2', () => {
    ...
    it('insufficient-gas attach uses fee-path gate (2.1 TON), not TOKSIM burn-path', () => {
        expect(MIN_TON_FEE_PATH_NANO).toBe(toNano('2.1'));
        ...
        expect(
            checks.some(
                (c) =>
                    c.message.includes('2.1') || c.message.includes(String(MIN_TON_FEE_PATH_NANO)),
            ),
        ).toBe(true);
    });
