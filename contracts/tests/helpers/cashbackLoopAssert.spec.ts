...
function mockInternalTx(from: Address, to: Address, emptyBody: boolean, exitCode = 0): RelayTransactions[number] {
    return {
        inMessage: {
            info: {
                type: 'internal',
...
describe('partner wrappers', () => {
    ...
    it('countEmptyProposalStakingHops delegates to generic helper', () => {
        const txs: RelayTransactions = [mockInternalTx(ADDR_C, ADDR_B, true), mockInternalTx(ADDR_B, ADDR_C, false)];
...
describe('assertRelayFlowClean', () => {
    it('passes for a clean flow within bounds and zero partner hops', () => {
        const txs: RelayTransactions = [mockInternalTx(ADDR_A, ADDR_B, false), mockInternalTx(ADDR_B, ADDR_A, false)];
        expect(() => assertRelayFlowClean(txs, { maxTx: 15, partnerPairs: [[ADDR_A, ADDR_B]] })).not.toThrow();
    });

    it('fails when empty-body partner hops exist', () => {
        const txs: RelayTransactions = [mockInternalTx(ADDR_A, ADDR_B, true)];
        expect(() => assertRelayFlowClean(txs, { partnerPairs: [[ADDR_A, ADDR_B]] })).toThrow();
    });

    it('fails when tx count exceeds maxTx', () => {
        const txs: RelayTransactions = Array.from({ length: 16 }, () => mockInternalTx(ADDR_A, ADDR_C, false));
