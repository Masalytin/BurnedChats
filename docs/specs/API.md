...
PoW is enforced on `/app/session.create` and `/app/dmInvite.mint`. Challenges for `search` / `room_create` / `invite` are not issued until those routes are gated. Issued difficulty is calibrated to the on-device WebView budget, not miner economics.
