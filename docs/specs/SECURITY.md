...
- **Read-only (IMP-ROOM-11):** флаг `readOnly` в `room:{roomId}`; при `true` только owner может отправлять (`ROOM_READ_ONLY` для остальных членов). Участники по-прежнему получают fan-out на topic. Co-admin в read-only — **IMP-ROOM-14**.
- **Роли (IMP-ROOM-13):** источник истины владельца — `room.ownerInternalId`. Overlay `room_roles:{roomId}` хранит только `admin` \| `member` (отсутствие записи = member). `roleOf(roomId, internalId)` → `owner` \| `admin` \| `member`. **Transfer ownership** (`/app/room.transferOwnership`, owner-only): `newOwnerInternalId` должен быть текущим членом; атомарно меняется `room.ownerInternalId`, предыдущий владелец получает overlay `admin`, у нового владельца overlay удаляется; broadcast `ROOM_OWNERSHIP_TRANSFERRED` на `/topic/room/{roomId}`. **Rekey не запускается** — групповой ключ не меняется, новый владелец уже держит ключ как член. Owner-only остаются: burn, transfer ownership, setRole (IMP-ROOM-14). Co-admin права на kick/invite/mute — **IMP-ROOM-14**.
